...
export function ProductCard({
  ...
  ...raw
}: ProductCardProps & Record<string, any>) {
  const { t } = useLanguage()

  // --- Compatibility: Home/Categories pass DB snake_case fields via `{...product}`.
  // Keep UI unchanged; only normalize values used for flash sale + pricing.
  ...
  const resolvedPiAmount =
    piAmount ?? Number(raw.pi_amount ?? raw.price_pi ?? raw.price ?? 0)

  const resolvedPitdAmount =
    pitdAmount ?? Number(raw.pitd_amount ?? raw.price ?? 0)

  const resolvedFlashSaleEnabled = Boolean(
    flashSaleEnabled ?? raw.flash_sale_enabled ?? raw.flashSaleEnabled ?? false,
  )

  const resolvedFlashSaleStartDate = toDate(flashSaleStartDate ?? raw.flash_sale_start_date)
  const resolvedFlashSaleEndDate = toDate(flashSaleEndDate ?? raw.flash_sale_end_date)

  const resolvedFlashSalePiPrice =
    flashSalePiPrice ??
    (raw.flash_sale_pi_price !== undefined && raw.flash_sale_pi_price !== null
      ? Number(raw.flash_sale_pi_price)
      : raw.flash_sale_pi_amount !== undefined && raw.flash_sale_pi_amount !== null
        ? Number(raw.flash_sale_pi_amount)
        : raw.flash_sale_price_pi !== undefined && raw.flash_sale_price_pi !== null
          ? Number(raw.flash_sale_price_pi)
          : undefined)

  const resolvedFlashSalePitdPrice =
    flashSalePitdPrice ??
    (raw.flash_sale_pitd_price !== undefined && raw.flash_sale_pitd_price !== null
      ? Number(raw.flash_sale_pitd_price)
      : raw.flash_sale_pitd_amount !== undefined && raw.flash_sale_pitd_amount !== null
        ? Number(raw.flash_sale_pitd_amount)
        : raw.flash_sale_price_pitd !== undefined && raw.flash_sale_price_pitd !== null
          ? Number(raw.flash_sale_price_pitd)
          : undefined)

  const resolvedOriginalPiAmount =
    originalPiAmount ?? (resolvedFlashSaleEnabled ? resolvedPiAmount : undefined)

  const resolvedOriginalPitdAmount =
    originalPitdAmount ?? (resolvedFlashSaleEnabled ? resolvedPitdAmount : undefined)
  const debugFlash =
    typeof window !== "undefined" && new URLSearchParams(window.location.search).get("debugFlash") === "1"
  ...
  const flashSaleIsActive = isFlashSaleActive(resolvedFlashSaleEnabled, resolvedFlashSaleStartDate, resolvedFlashSaleEndDate)

  const displayPiAmount =
    flashSaleIsActive && resolvedOriginalPiAmount
      ? getFlashSalePrice(resolvedOriginalPiAmount, resolvedFlashSalePiPrice, flashSaleDiscountPercent)
      : resolvedPiAmount

  const displayPitdAmount =
    flashSaleIsActive && resolvedOriginalPitdAmount
      ? getFlashSalePrice(resolvedOriginalPitdAmount, resolvedFlashSalePitdPrice, flashSaleDiscountPercent)
      : resolvedPitdAmount

  const calculatedDiscount =
    flashSaleIsActive && resolvedOriginalPiAmount && displayPiAmount
      ? calculateFlashSaleDiscount(resolvedOriginalPiAmount, displayPiAmount)
      : discountPercentage

  useEffect(() => {
    if (!flashSaleIsActive || !resolvedFlashSaleEndDate) return

    const updateTimer = () => {
      const now = new Date()
      const diff = resolvedFlashSaleEndDate.getTime() - now.getTime()
  ...
  }, [flashSaleIsActive, resolvedFlashSaleEndDate, t])
  ...
  return (
    ...
    >
      ...
      <div className="aspect-square relative bg-gradient-to-br from-purple-100 to-pink-100">
        ...
        {flashSaleIsActive && resolvedFlashSaleEndDate && (
          <div
      ...
      <div className={compact ? "p-1.5 space-y-0.5" : "pt-0.5 pb-1.5 px-2 space-y-1"}>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {supportsPi && displayPiAmount !== undefined && displayPiAmount > 0 && (
            <div className="space-y-0.5">
              {flashSaleIsActive && resolvedOriginalPiAmount && originalPiAmount !== displayPiAmount && (
                <div
          ...
          {supportsPitd && displayPitdAmount !== undefined && displayPitdAmount > 0 && (
            <div className="space-y-0.5">
              {flashSaleIsActive && resolvedOriginalPitdAmount && originalPitdAmount !== displayPitdAmount && (
                <div
        ...
        {debugFlash && (
          <div className="mt-2 text-xs text-gray-500">
            flashSaleEnabled={String(resolvedFlashSaleEnabled)} | nowPi={String(displayPiAmount ?? resolvedPiAmount)} |
            nowPITD={String(displayPitdAmount ?? resolvedPitdAmount)} | flashPi={String(resolvedFlashSalePiPrice)} |
            flashPITD={String(resolvedFlashSalePitdPrice)} | start={String(resolvedFlashSaleStartDate)} |
            end={String(resolvedFlashSaleEndDate)}
          </div>
        )}
</Card>
  )
