...
export default function PitdManagementPage() {
  ...
  async function loadData() {
    try {
      ...
      const adminUsername = (user as any)?.username || ""
      ...
      // Transactions (admin history) MUST go through server API (no anon PITD reads)
      ...
      try {
        ...
        const res = await fetch(`/api/admin/pitd-history?limit=50`, { headers })
        const json = await res.json().catch(() => ({}))
        if (res.ok && Array.isArray(json.transactions)) {
          transactionsData = json.transactions
        } else {
          console.warn("[Admin] pitd-history failed", json)
        }
      } catch (e) {
        console.warn("[Admin] pitd-history error", e)
      }
      ...
      // Admin history is already returned in a UI-friendly shape from /api/admin/pitd-history
      // (type: grant/revoke, amount as number, username resolved).
