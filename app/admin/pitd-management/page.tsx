  ...
  async function loadData() {
    try {
      ...
      if (usersData) {
        ...
        // If there are historical duplicate rows for the same Pi account, dedupe for display.
        const dedupedUsers = filteredUsers.filter((u: any, idx: number, arr: any[]) => {
          const key = (u.pi_uid || "") + "::" + (u.pi_username || "")
          return arr.findIndex((x: any) => ((x.pi_uid || "") + "::" + (x.pi_username || "")) === key) === idx
        })

        console.log("[v0] After filter:", filteredUsers.length, "users")
        if (dedupedUsers.length !== filteredUsers.length) {
          console.log("[v0] After dedupe:", dedupedUsers.length, "users")
        }
        setDebugInfo((prev) => `${prev} | After filter: ${filteredUsers.length} users` + (dedupedUsers.length !== filteredUsers.length ? ` | After dedupe: ${dedupedUsers.length}` : ""))

        setUsers(
          dedupedUsers.map((u: any) => ({
            id: u.id,
            pi_uid: u.pi_uid || "",
