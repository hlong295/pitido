...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch("/api/_diag", { cache: "no-store" })
        const data = await res.json().catch(() => ({ parseError: true }))
        if (!cancelled) setDiag({ ok: res.ok, status: res.status, ...data })
      } catch (e: any) {
        if (!cancelled) setDiag({ ok: false, error: String(e?.message || e) })
      }
    })()
    return () => {
      cancelled = true
              ...
              <CardContent className="space-y-4">
                ...
                {dbg && (
                  <div className="rounded-lg border bg-card p-3 text-xs">
                    <div className="font-semibold mb-2">DBG snapshot (login)</div>
