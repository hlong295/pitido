...
export default function AddProductPage() {
  ...
  // Only Provider (or Admin) can access posting features.
  useEffect(() => {
    if (!user) return
    if (isAdmin()) return
...
const loadCategories = async () => {
  ...
  const loadCurrentUserDbId = async () => {
    ...
    try {
      // In our app, `user.uid` is the internal UUID (public.pi_users.id) in most flows.
      // Some older flows may still store `pi_uid`. So we try both safely.
      console.log("[v0] Loading current user DB ID:", user.uid)

      let data: any = null

      // 1) Try by primary key id
      const byId = await supabase.from("pi_users").select("id").eq("id", user.uid).maybeSingle()
      if (byId?.data?.id) data = byId.data

      // 2) Fallback: try by pi_uid
      if (!data?.id) {
        const byPiUid = await supabase.from("pi_users").select("id").eq("pi_uid", user.uid).maybeSingle()
        if (byPiUid?.data?.id) data = byPiUid.data
      }
      ...
      if (data?.id) {
        ...
        console.warn("[v0] Could not resolve current user DB ID")
      }
    } catch (error) {
