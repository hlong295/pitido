...
export async function POST(request: Request) {
  const debugEnabled = (() => {
    try {
      ...
      // Support both ?debug=1 (older) and ?dbg=1 (used in UI)
      return u.searchParams.get("debug") === "1" || u.searchParams.get("dbg") === "1"
    } catch {
      return false
    }
  })()
  ...
  try {
    // Keep the raw body because downstream logic reads extra flags.
    ...
    const piUsername = (body?.piUsername || body?.pi_username || metadata?.pi_username || metadata?.piUsername || metadata?.username)
      ? String(body?.piUsername || body?.pi_username || metadata?.pi_username || metadata?.piUsername || metadata?.username)
      : undefined
