...
export async function POST(request: Request) {
  ...
  try {
    ...
    // NOTE: schema pitd_transactions (per your DB) does NOT have "status" column.
    // Keep the insert strictly aligned to existing columns, otherwise insert will fail
    // and the UI will show "no transactions" even though wallet balance updated.
    // IMPORTANT: transaction_type must match the DB check constraint.
    // Your DB already shows values like "admin_grant" in public.pitd_transactions.
    // So we must write those exact values to avoid violating the check constraint.
    const transactionType = actionType === "grant" ? "admin_grant" : "admin_revoke"
    console.log("[v0] Creating transaction, type:", transactionType)
    ...
    try {
      const insertPayload = {
        ...
        // Use the master admin id as reference_id for auditing.
        reference_id: masterAdminId,
        reference_type: "admin",
        metadata: {
          action: actionType,
          admin_user_id: masterAdminId,
          // Raw requester identity (may be uuid/username depending on client env)
          requester_identity_raw: requesterIdentityRaw,
          target_user_id: targetUserId,
        },
      }

      const res = await supabaseAdmin.from("pitd_transactions").insert(insertPayload as any)
      txError = (res as any)?.error || null
    } catch (e: any) {
      // This covers runtime exceptions (e.g., bad payload construction) so we can rollback.
      txError = { message: e?.message || String(e) }
    }

    if (txError) {
      console.log("[v0] Transaction creation failed:", txError.message)
